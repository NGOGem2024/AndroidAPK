...
const fetchItems = useCallback(async (showLoader = true) => {
  ...
  const renderHeaderImage = useCallback(() => {
    let imageSource;
    if (typeof subcategoryImage === 'string') {
      imageSource = { uri: subcategoryImage };
    } else if (subcategoryImage) {
      imageSource = subcategoryImage;
    } else {
      // For React Native CLI, we need to require images statically
        ...
        <View style={styles.itemContent}>
          <View style={styles.itemMainInfo}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleViewDetails(item)}
            >
              <Ionicons name="arrow-forward-outline" size={20} color="#2196f3" />
            </TouchableOpacity>
          </View>
        </View>
      </TouchableOpacity>
